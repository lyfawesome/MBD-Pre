...
- 部件 75/76 的争议来源位于刚体对应阶段。补充诊断证明二者关于 `x=0` 精确镜像：正旋转不存在，反射后完整顶点、类型化边、面边界及双向 Boolean 差均通过。因此它们在 `SE(3)` 下不同、在 `O(3)` 下相同；完整证据见 [cases/MOTOR_PARTS_75_76.md](cases/MOTOR_PARTS_75_76.md)。
...
1. 为 38 件争议组建立带人工标签的最小回归夹具；为 75/76 增加 `mirror_equivalent` 回归夹具，同时保留通用的 `indeterminate` 选项。
